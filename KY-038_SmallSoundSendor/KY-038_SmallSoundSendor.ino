 ...
 * Lesson - [KY-038] Small Sound Sensor
 *
 ...
 *
 * KY-038 is a module that is designed to detect sound intensity and convert
 ...
 *+-+--+--+--+--+--+--+--+--+--+--+--+--+--+--+--+--+--+--+--+
 *|          CALIBRATION
 *| The KY-038 Small Sound Sensor functions like a thermostat
 *| for sound. 
 *| 
...
const uint8_t KY_038_DIGITAL_IN = 8;  // digital sensor
const uint8_t KY_038_ANALOG_IN = A0;  // analog sensor

void setup() {
  Serial.begin(9600);
  pinMode(LED_BUILTIN, OUTPUT);      // define LED_BUILTIN as output interface
  pinMode(KY_038_DIGITAL_IN, INPUT); // define digital pin as input interface
  pinMode(KY_038_ANALOG_IN, INPUT);  // define analog pin as input interface
}

void loop() {
  int ky038_Dval = digitalRead(KY_038_DIGITAL_IN); // Reads digital value
  int ky038_Aval = analogRead(KY_038_ANALOG_IN);   // Reads analog value

  if (ky038_Dval == HIGH) // temperature GREATER than set threshold
  ...
  // Serial.print(ky038_Dval);
  // Serial.print(" | ");
  // Serial.print("Analog value: ");
  // Serial.println(ky038_Aval);
}
...
const uint8_t KY_024_DIGITAL_IN = 8; // digital sensor
const uint8_t KY_024_ANALOG_IN = A0; // analog sensor

void setup() {
  Serial.begin(9600);
  pinMode(LED_BUILTIN, OUTPUT);      // define LED_BUILTIN as output interface
  pinMode(KY_024_DIGITAL_IN, INPUT); // define digital pin as input interface
  pinMode(KY_024_ANALOG_IN, INPUT);  // define analog pin as input interface
}

void loop() {
  int ky024_Dval = digitalRead(KY_024_DIGITAL_IN); // Reads digital value
  int ky024_Aval = analogRead(KY_024_ANALOG_IN);   // Reads analog value
